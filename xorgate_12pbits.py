# -*- coding: utf-8 -*-
"""
Created on Sat Jun 11 20:52:40 2022
...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 11 15:48:23 2022
...
#Ic=0.002*1e-9
Ic=0.001
P_SHE=1.5#(AMTJ/ASHM)*theta_SHE*(1-(1/np.cosh(t_SHM/lambda_sf)))
...
class pbit:
	...
	def LLGS(self,m,H):
		Hth=np.sqrt(2*alpha*k_b*300/((1+alpha**2)*abs(gamma)*mu_0*Ms*V*delta_t))*np.array([np.random.normal(0,1),np.random.normal(0,1),np.random.normal(0,1)])*2000
		H=-4*np.pi*Ms*m[2]*np.array([0,0,1])+Hth+(2*k_u/(mu_0*Ms))*np.dot(m,u)*u
...
# pos=0
# for t in range(501):
#     if Aaa[t]<mini and a.mz_array[t]==1 and b.mz_array[t]==1:
#         mini=Aaa[t]
#         pos=t
...
#plt.close()
G0=100
G=np.array([[0,0,0,0],[0,G0,G0,2*G0],[0,G0,G0,2*G0],[0,2*G0,2*G0,2*G0]])
Vout=np.array([0.5,0.5,0.5,0.5])
Vbias=np.array([-0.5,-0.5,-0.5,-0.5,0.5])
Iin=np.array([0,(G[1][2]+G[1][3])*Vout[1]+G[1][1]*Vbias[1],(G[2][1]+G[2][3])*Vout[2]+G[2][2]*Vbias[2],(G[3][1]+G[3][2])*Vout[3]+G[3][3]*Vbias[3]])

Aa=andgate(np.array([[0,-1,2],[-1,0,2],[2,2,0]]),np.array([1,1,-2]))
Ab=orgate(np.array([[0,-1,2],[-1,0,2],[2,2,0]]),-np.array([1,1,-2]))
Ac=xorgate(Aa,Ab)

Aaa=[]
#Aaa=np.append(Aaa,Aa.E(1))
Ic=Iin[1]*5*1e-18*Iin[1]
Is=P_SHE*Ic
a=pbit([],[],[])
a.solve()
Ic=Iin[2]*5*1e-18*Iin[1]
Is=P_SHE*Ic
b=pbit([],[],[])
b.solve()
Ic=Iin[3]*5*1e-18*Iin[1]
Is=P_SHE*Ic
ca=pbit([],[],[])
ca.solve()
Aaa1=[]
...
Ic=Iin[1]*5*1e-18*Iin[2]
Is=P_SHE*Ic
a=pbit([],[],[])
a.solve()
Ic=Iin[2]*5*1e-18*Iin[2]
Is=P_SHE*Ic
b=pbit([],[],[])
b.solve()
Ic=Iin[3]*5*1e-18*Iin[2]
Is=P_SHE*Ic
cb=pbit([],[],[])
cb.solve()
...
for t in range(501):
    if Aaa2[t]<=mini and c3.mz_array[t]==1:# c.mz_array[t]==-1 and Aaa[t]<=mini:
        ...
        bb.append(b.mz_array[t])
        cc.append(c3.mz_array[t])
        e[t]-=c3.mz_array[t]
        if c3.mz_array[t]==1:
            x[1]+=1
        elif c3.mz_array[t]==-1:
            x[0]+=1
        
        if a.mz_array[t]==-1 and b.mz_array[t]==-1:
            a1.append(0)
        elif a.mz_array[t]==1 and b.mz_array[t]==-1:
            a1.append(1)
        elif a.mz_array[t]==-1 and b.mz_array[t]==1:
            a1.append(2)
        elif a.mz_array[t]==1 and b.mz_array[t]==1:
...
for t in range(len(aa)):
    if aa[t]==-1 and bb[t]==-1 and cc[t]==-1:
        a2.append(0)
    elif aa[t]==-1 and bb[t]==-1 and cc[t]==1:
        a2.append(1)
    elif aa[t]==-1 and bb[t]==1 and cc[t]==-1:
        a2.append(2)
    elif aa[t]==-1 and bb[t]==1 and cc[t]==1:
